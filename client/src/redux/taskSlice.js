...
export const addTask = (task, id) => async (dispatch) => {
	...
	const response = await axios.post('https://task-management-hrbz.onrender.com/task/add', taskData);
	if (response) {
...
export const getAllTasks = (token, id) => async (dispatch) => {
	...
	try {
		const response = await axios.get(
			'https://task-management-hrbz.onrender.com/task/tasks',
			config
		);
...
export const arrowClick = (item, string) => async () => {
	...
	try {
		let response = await axios.put(
			`https://task-management-hrbz.onrender.com/task/${taskData.id}`,
			taskData
		);
...
export const deleteItem = (id) => async (dispatch) => {
	let res = await axios.delete(`https://task-management-hrbz.onrender.com/task/${id}`);
